...
$2n^{2}\theta - n^{2}\bar{X_n} - n^{2}\bar{X_n}$  
...
On cherche à écrire un programme permettant de calculer l'estimateur $\hat \alpha_n$ de $\alpha_0 =E(Y)$. On a calculé deuc estimateurs de $\alpha_0$ tels que $\hat \alpha1_n= \sum \hat m(X_i)/n$ et $\hat \alpha2_n= \frac {1}{n}\sum [\frac{D_i Y_i}{\hat p(X_i)}+(1-\frac {D_i}{\hat p(X_i)})\hat m(X_i)]$
...
Il suffit ensuite de construire les estimateurs $\hat\alpha1_n$ et $\hat \alpha2_n$ avec ces valeurs

Avec h=5 on obtient $\hat \alpha1_n$ =`r alpha1_n` et $\hat \alpha2_n$= `r alpha2_n` 
...
En appliquant à la dernière fonction plusieurs séquences de h différents, on essaye de determiner une approximation de h optimale c-a-d la valeur de h pour laquelle $h_{CV}$ est minimal. On trouve donc pour $h_{CV}$ la valeur 5.09

On remplace ainsi ensuite la valeur h=5 par h=1.069 pour $\hat p$ et h=5.08 pour $\hat m$ dans les fonctions permettant de calculer ces estimateurs

```{r, echo= FALSE}
h_validcroisee=1.069
NW<-function(x,h=5.08){
  a<-c()
  kernel<-c()
...
On obtient $\hat \alpha1_n$ =`r theta_tild1` et $\hat \alpha2_n$= `r theta_tild2` ce qui est proche de la moyenne empirique.

Concentrons nous sur $\hat \alpha1_n$ et appliquant la loi des grands nombres pour voir s'il est asymptotiquement efficace. 
